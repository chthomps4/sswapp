...
export const automationRecipes: AutomationRecipeConfig[] = [
  ...
  {
    id: "recipe-weekly-workflow-gap-audit",
    name: "Weekly Workflow Gap Audit",
    slug: "weekly-workflow-gap-audit",
    description: "Weekly operations smoke alarm for stalled workflows, failed automations, stale approvals, import/export issues, and safety gaps.",
    triggerType: "scheduled",
    active: true,
    stepsJson: [
      "load date range",
      "load app config status",
      "run automation health checkpoint",
      "run content workflow checkpoint",
      "run approval checkpoint",
      "run image prompt and asset checkpoint",
      "run export checkpoint",
      "run social import checkpoint",
      "run metrics and insights checkpoint",
      "run brand and platform coverage checkpoint",
      "run campaign and offer checkpoint",
      "run prompt and recipe checkpoint",
      "run data integrity checkpoint",
      "run privacy and config checkpoint",
      "score health",
      "create gaps and action items",
      "generate deterministic summary",
    ],
    requiredInputsJson: ["dateRangeStart", "dateRangeEnd"],
    optionalInputsJson: ["includeAiSummary", "includeArchived", "dryRun"],
    defaultSettingsJson: { dateRangeDays: 7, manualRunEnabled: true, scheduledEnabled: false, aiSummary: false },
    featureFlagsJson: {
      weeklyAudit: "ENABLE_WEEKLY_WORKFLOW_GAP_AUDIT",
      cronEnabled: "WEEKLY_WORKFLOW_AUDIT_CRON_ENABLED",
      aiSummary: "ENABLE_AI_WORKFLOW_AUDIT_SUMMARY=false",
      autoApproval: "ENABLE_AUTO_APPROVAL=false",
      autoPublishing: "ENABLE_AUTO_PUBLISHING=false",
      rawPrivateData: "blocked",
    },
    createdAt: now,
    updatedAt: now,
  },
