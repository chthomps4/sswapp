...
import type { Brand, PostVariant } from "@/lib/types";
...
export function SswDashboard({ brands, posts, dashboard }: DashboardProps) {
  ...
  const [generationNote, setGenerationNote] = useState("Run Today creates persisted drafts, image prompts, approvals, and an automation run.");
  ...
  const canExportPack = dashboard.statusMode === "persistent";
  ...
  async function generatePack() {
    setGenerationNote("Running today's persisted content operation...");
    const response = await fetch("/api/automations/run-today", {
      ...
      body: JSON.stringify({
        ...
        dailyTheme: theme,
        strategicPriority: researchBrief || "Create useful daily visibility without adding operating drag.",
        selectedBrands: [activeBrand],
        selectedPlatforms: ["linkedin", "facebook", "instagram", "google-business-profile"],
        businessNotes: researchBrief,
        includeImagePrompts: true,
        includeCarouselOutlines: true,
        includeGoogleBusinessProfile: true,
        includeReddit: false,
        includeShortVideoIdeas: true,
        includePerformanceContext: true,
      }),
    });
    const data = (await response.json().catch(() => null)) as { message?: string; packUrl?: string } | null;
    if (!response.ok || !data?.packUrl) {
      setGenerationNote(data?.message || "Run Today failed. Open the Run Today page for the full form and retry.");
      return;
    }
    setGenerationNote(data.message || "Run Today complete. Opening the generated pack.");
        ...
        <main className="min-w-0 px-4 py-5 sm:px-6 lg:px-8">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              {canExportPack ? (
                ...
                <span className="inline-flex items-center gap-2 rounded-md border border-amber-200 bg-amber-50 px-3 py-2 text-sm font-medium text-amber-900">
                  <FileText size={16} />
                  Exports unlock after Run Today
                </span>
              )}
            </div>
          </header>
          ...
          <section className="grid gap-4 py-5 xl:grid-cols-3">
            ...
            <div id="metrics" className="rounded-lg border border-stone-200 bg-white p-4 shadow-sm">
              ...
              <p className="mt-4 text-sm text-stone-600">Best current pillar: {report.bestPillars[0]?.pillar || "Waiting for confirmed metrics"}</p>
          ...
          <section className="grid gap-4 pb-5 xl:grid-cols-[minmax(0,1fr)_360px]">
            <div className="rounded-lg border border-stone-200 bg-white p-4 shadow-sm">
              <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
                <div>
                  ...
                  <p className="text-sm text-stone-600">Primary actions for getting the private dashboard into a working daily rhythm.</p>
                </div>
              </div>
              <div className="mt-4 grid gap-3 md:grid-cols-2">
                {[
                  ["Run Today", "/run-today", "Create a persisted content pack, drafts, image prompts, approvals, and automation run."],
                  ["Approval Queue", "/approvals", "Review generated copy and image prompts before any export."],
                  ["Social Imports", "/social/imports/new", "Upload dashboard CSVs so metrics become useful."],
                  ["Prompt Library", "/prompts", "Inspect templates and render previews."],
                ].map(([label, href, description]) => (
                  <a key={label} href={href} className="rounded-md border border-stone-200 p-3 hover:border-emerald-300">
            ...
            <div className="rounded-lg border border-stone-200 bg-white p-4 shadow-sm">
              ...
              <p className="mt-1 text-sm text-stone-600">Current operating inputs. Nothing here publishes or approves content.</p>
              <div className="mt-4 space-y-2 text-sm">
                {[
                  `${dashboard.totalDrafts} drafts loaded`,
                  `${dashboard.pendingApprovalCount} approvals pending`,
                  `${dashboard.socialImportCount} social imports recorded`,
                  `${dashboard.socialInsightCount} insights available`,
                ].map((line) => (
